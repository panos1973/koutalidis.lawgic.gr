import CreateNewFolder from "@/components/vault/new_folder";
import { NextPage } from "next";
import { getTranslations } from "next-intl/server";
import { getUserFileStats, getVaultFolders } from "../actions/vault_actions";

import { auth } from "@clerk/nextjs/server";
import { checkUsageLimits } from "../actions/subscription";

import VaultFolders from "@/components/vault/folders";
...
const Page = async () => {
  ...
  const folders = await getVaultFolders(userId!, organizationId);
  ...
  return (
    <div className="px-4 md:px-12">
      <div className=" w-full md:w-[70%] h-[90%]">
        <div className=" py-6">
          <h1 className="text-xl font-bold">{t("yourPersonalVault")}</h1>
