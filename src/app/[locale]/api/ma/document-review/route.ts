...
export async function POST(req: Request) {
  try {
    ...
    const basePrompt = getToolPrompt(toolId || 'spa-review')
    const maxTokens = getToolMaxTokens(toolId || 'spa-review')
    ...
    console.error('M&A document review error:', error.message)
    return new Response(
