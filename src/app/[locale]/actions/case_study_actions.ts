...
export const saveCaseMessage = async (
  ...
  const previousMessage = await db.query.case_study_messages.findFirst({
    where: (case_study_messages, { eq, and }) =>
      and(
        eq(case_study_messages.case_study_id, caseStudyId),
        eq(case_study_messages.content, message.trim()),
        eq(case_study_messages.role, role)
      ),
    orderBy: (case_study_messages, { desc }) => [
      desc(case_study_messages.createdAt),
    ],
  })
  ...
  const saveMessageWithRetry = async () => {
    try {
      ...
      console.error(
        `Failed to save case message (attempt ${retryCount + 1}):`,
        error
      )
      ...
      throw new Error(
        `Failed to save case message after ${maxRetries} attempts: ${error.message}`
      )
    }
  }
...
export const searchCaseDocuments = async (
  userId: string,
  caseStudyId: string,
  query: string,
  k: number = 5
  ...
  // Filter for documents related to this case study
  return results.filter((doc) => doc.metadata.caseStudyId === caseStudyId)
}
