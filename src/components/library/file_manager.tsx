...
import UploadLibraryFile from "./upload_files";
...
const LibraryFileManager: NextPage<Props> = ({
  folderId,
  files,
  subscriptionData,
  locale,
  ...
  useEffect(() => {
    const fetchFolders = async () => {
      if (user?.id) {
        const folderData = await getLibraryFolders(user.id);
  ...
  const handleMoveFiles = async () => {
    ...
    setLoading(true);
    try {
      await moveLibraryFiles(selectedFiles, targetFolderId);
  ...
  const handleDownloadFiles = async () => {
    ...
    try {
      for (const fileId of selectedFiles) {
        const fileData = await downloadLibraryFile(fileId);
  ...
  return (
    <div className="py-6">
      ...
      <div className="grid grid-cols-5 gap-4">
        <UploadLibraryFile folderId={folderId} subData={subscriptionData} />
        {files.map((file) => (
          <div
            key={file.id}
            ...
            {!selectionMode && (
              <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <Button
                  ...
                  onClick={async () => {
                    try {
                      const fileData = await downloadLibraryFile(file.id);
                      if (fileData?.success && fileData?.fileUrl) {
                        const link = document.createElement("a");
                        link.href = fileData.fileUrl;
                        link.download = fileData.fileName || "download";
