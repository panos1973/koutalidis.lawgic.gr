...
const ChatLinks: NextPage<Props> = ({ chats, chatHistoryTranslations }) => {
  ...
  const renderChatContent = (chat: Chat) => (
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <div className="min-w-1/2">
            ...
            {chat.note && (
              <p className="text-xs text-[#c2032f] my-1">{chat.note}</p>
            )}
  ...
  const renderNoteEditingContent = (chat: Chat) => (
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <div className="min-w-1/2">
                  ...
                  <div
                    className={cn('py-2 px-2 space-y-10', {
                      ...
                      'bg-red-100 border-l-4 border-red-500':
                        isMassDelete && selectedChats.includes(chat.id),
                    })}
                    key={chat.id}
                  >
      ...
      <div className="md:block hidden">
        ...
        {chats.map((chat) => (
          ...
          >
            <div className="flex space-x-4 justify-between items-center w-full">
              {visibleNoteChatId === chat.id ? (
                <div className="w-full">{renderNoteEditingContent(chat)}</div>
              ) : isMassDelete ? (
                <div
                  className="w-full cursor-pointer"
                  onClick={() => handleChatSelection(chat.id)}
